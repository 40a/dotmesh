...
func consumePrelude(r io.Reader) (Prelude, error) {
	...
	for !finished {
		...
		buf = append(buf, b...)
		idx := bytes.Index(buf, END_DATAMESH_PRELUDE)
		if idx != -1 {
...
func encodePrelude(prelude Prelude) ([]byte, error) {
	...
	// is to avoid framing issues. This works because END_DATAMESH_PRELUDE has
	...
	encoded = append(encoded, END_DATAMESH_PRELUDE...)
	return encoded, nil
}
...
func mnt(fs string) string {
	...
	if mountPrefix == "" {
		...
		// MOUNT_PREFIX will be like /datamesh-test-pools/pool_123_1/mnt
		// and we want to return
		// /datamesh-test-pools/pool_123_1/mnt/dmfs/:filesystemId
...
// setup logfile
func setupLogging() {
	log.SetOutput(getLogfile("datamesh"))
}
